...
//CONTINUE HERE
  ...
  //CONTINUE HERE CONSIDER CHANGING ACCEL TO 6D VECTOR ALSO ACCOUNT FOR VARYING LENGTH OF PRISMATIC JOINT
  ...
  if(curjoint->isBase){
    ...
    b_vector[curjoint->jointID] = curjoint->S().dot(curjoint->wrench);    //is this the b value?? what about gravity?
  }
...
void eliminate_fixed_joints(Joint* curjoint){
  ...
  for(Joint* childjoint : childjoints_copy){
    if(childjoint->ispris){
      Eigen::Vector3d axis(childjoint->axis, 0.0, 0.0);
      childjoint->origin.xyz = axis*childjoint->angle;
    }
  ...
  if(!(curjoint->isrev || curjoint->ispris)){
...
//LF
  ...
  LF_HAA_rev.ispris = true;    //change to prismatic
  LF_HAA_rev.axis = 1;
